...
export const getAllProducts = async (req, res) => {
  ...
  try {
    let result;
    if (category) {
      result = await pool.query("SELECT * FROM products WHERE category = $1 LIMIT $2 OFFSET $3", [
        category,
        limit || 10,
        offset || 0,
      ]);
    } else {
      result = await pool.query("SELECT * FROM products LIMIT $1 OFFSET $2", [limit || 10, offset || 0]);
    }
